...
const SELECTION_TO_PICK_MODE = Object.freeze({
    ...
    [SelectionMode.LINE]: PickMode.EDGE,
...
const PICK_TO_SELECTION_MODE = Object.freeze({
    ...
    [PickMode.EDGE]: SelectionMode.LINE,
...
export class PickingController {
    constructor(sceneController, options = {}) {
        ...
        this.selectionFeatureAngle = Math.min(90, Math.max(0, options.selectionFeatureAngle ?? 20));
    ...
    _getSelectableActors() {
        ...
        this.sceneController.scene?.traverse?.((obj) => {
            if (obj?.isActor && obj.visible && obj.name !== GRID_NAME) {
                obj.selectionFeatureAngle = this.selectionFeatureAngle;
                actors.push(obj);
            }
        });
        return actors;
    ...
    setSelectionFeatureAngle(angle) {
        const next = Math.min(90, Math.max(0, Number(angle)));
        if (!Number.isFinite(next) || next === this.selectionFeatureAngle) return this;
        this.selectionFeatureAngle = next;
        this.sceneController.scene?.traverse?.((obj) => {
            if (!obj?.isActor || obj.name === GRID_NAME) return;
            obj.selectionFeatureAngle = next;
            ActorTopology.invalidate(obj);
        });
        this._setHover(null);
        this.clearSelection();
        this._emit("selectionfeatureanglechange", next);
        return this;
    }

    getSelectionMode() { return this.selectionMode; }
    getSelectionFeatureAngle() { return this.selectionFeatureAngle; }
