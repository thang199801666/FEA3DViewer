// Rendering/Renderer.js  — vtkRenderer
// Can create its own scene/camera, or ADOPT an existing shared scene and a
// pre-built vtkCamera (so it plugs into an app that already owns them).
import * as THREE from 'three';
import { Camera } from './Camera.js';

export class Renderer {
  constructor({
    background = 0x101014, viewport = [0, 0, 1, 1],
    scene = null, camera = null, addDefaultLights,
  } = {}) {
    const usingSharedScene = !!scene;
    this.scene = scene ?? new THREE.Scene();
    if (!usingSharedScene && background !== null) this.scene.background = new THREE.Color(background);
    ...
    // default off when adopting a shared (already-lit) scene
  ...
  get camera() { return this.activeCamera.getThreeCamera(); }
  setActiveCamera(cam) { this.activeCamera = cam; return cam; }
  ...
  getActorForObject(object3D) {
    let o = object3D;
    while (o) { if (this._objectToActor.has(o)) return this._objectToActor.get(o); o = o.parent; }
    return null;
  }
  ...
  updateCameraAspect(aspect) { this.activeCamera.setAspect(aspect); }
