// Filters/ClipFilter.js
// Mô phỏng vtkClipPolyData với mặt phẳng cắt (implicit plane):
// giữ lại phần mesh nằm phía pháp tuyến dương của mặt phẳng, cắt chính xác
// các tam giác giao với mặt phẳng và nội suy toàn bộ point arrays.
//
// Lưu ý: output nhân bản đỉnh theo từng tam giác (không share) để đơn giản —
// đủ tốt cho hiển thị, không tối ưu cho phân tích topology.
...
export class ClipFilter extends Filter {
    constructor() {
        ...
        this.normal = [1, 0, 0];
        this.origin = [0, 0, 0];
        this.insideOut = false; // true = giữ phía ngược lại
    }
    ...
    getOutputData() {
        const pd = this.input;
        if (!pd) throw new Error("ClipFilter: chưa có input");
        ...
        // Các point arrays cần nội suy
        const srcArrays = [...pd.pointData.arrays.values()];
        const outArrays = srcArrays.map(a => ({ src: a, vals: [] }));
        const outPts = [];
        const outPolys = [];

        // Thêm 1 đỉnh output: hoặc copy đỉnh gốc (t=null) hoặc nội suy giữa i0,i1 theo t
        ...
        for (let t = 0; t < tris.length; t += 3) {
            ...
            if (nIn === 0) continue; // loại bỏ hoàn toàn
            ...
            // Cắt tam giác bằng Sutherland–Hodgman -> đa giác 3 hoặc 4 đỉnh
            const poly = [];
            for (let e = 0; e < 3; e++) {
                const a = e, b = (e + 1) % 3;
                if (inside[a]) poly.push(emit(idx3[a]));
                if (inside[a] !== inside[b]) {
                    const tt = d[a] / (d[a] - d[b]); // điểm giao trên cạnh a->b
                    poly.push(emit(idx3[a], idx3[b], tt));
                }
            }
            if (poly.length >= 3) outPolys.push(poly); // mapper sẽ fan-triangulate
        }
