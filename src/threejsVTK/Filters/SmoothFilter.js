// Filters/SmoothFilter.js
// Mô phỏng vtkSmoothPolyDataFilter (Laplacian smoothing):
// dịch mỗi đỉnh về phía trung bình các đỉnh lân cận, lặp nhiều lần.
// Giữ nguyên topology và toàn bộ pointData.
...
export class SmoothFilter extends Filter {
    constructor() {
        super();
        this.iterations = 20;
        this.relaxationFactor = 0.1; // 0..1, càng lớn càng mượt mạnh
    }
    ...
    getOutputData() {
        const pd = this.input;
        if (!pd) throw new Error("SmoothFilter: chưa có input");

        const nPts = pd.getNumberOfPoints();
        const tris = pd.getTriangles();

        // Xây danh sách kề (adjacency) từ các cạnh tam giác
