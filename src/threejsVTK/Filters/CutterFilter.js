// Filters/CutterFilter.js  (đã sửa)
// Tạo MẶT CẮT (cross-section) của mesh bề mặt bởi một mặt phẳng.
//   - fill=false: chỉ trả về đường viền (out.lines) — các vòng kín để vẽ contour.
//   - fill=true : bù lấp mặt cắt (out.polys) + đồng thời vẫn xuất out.lines (boundary edge)
//                 nếu edges=true. Mang theo scalars nội suy để tô contour.
//
// Những chỗ đã sửa so với bản cũ (nguyên nhân "mặt cắt bị lỗi"):
//   1) WELD theo spatial-hash có dung sai thực sự (thay vì Math.round(p/eps) dễ để
//      hai điểm rất gần nhau rơi vào 2 ô làm tròn khác nhau -> vòng KHÔNG đóng ->
//      earcut lấp bậy -> sinh tam giác vụn như trong ảnh).
//   2) DEDUPE các đoạn giao trùng nhau (đỉnh nằm đúng trên mặt phẳng / cạnh chung
//      giữa 2 tam giác sinh ra đoạn lặp -> đỉnh có bậc 4 -> ghép vòng đi lộn nhánh).
//   3) GHÉP VÒNG theo GÓC (face-tracing) thay vì "lấy neighbor đầu tiên", nên tại
//      giao điểm chạm nhau giữa các vòng lồng/răng cưa vẫn tách đúng vòng.
//   4) fill=true vẫn XUẤT boundary edge (out.lines) -> mặt cắt có viền.
//   5) Gắn Normals = pháp tuyến mặt phẳng cho point (shading nhất quán).
...
export class CutterFilter extends Filter {
    constructor() {
        ...
        this.normal = [1, 0, 0];
        this.origin = [0, 0, 0];
        this.fill = true;          // true = bù lấp thành mặt; false = chỉ đường viền
        this.passData = true;      // nội suy point arrays lên mặt cắt
        this.edges = true;         // MỚI: xuất out.lines (boundary edge) kể cả khi fill
        this.computeNormals = true;// MỚI: gắn point normal = pháp tuyến mặt phẳng
    }
    ...
    getOutputData() {
        const pd = this.input;
        if (!pd) throw new Error("CutterFilter: chưa có input");
        ...
        // Dung sai hàn: theo đường chéo bounding box.
        const b = pd.getBounds();
        const diag = Math.hypot(b[3] - b[0], b[4] - b[1], b[5] - b[2]) || 1;
        const eps = Math.max(diag * 1e-6, 1e-9);
        const eps2 = eps * eps;

        // ---- WELD bằng spatial-hash (dung sai thật, tránh khe hở do làm tròn) ----
        const wPos = [];                       // [x,y,z,...]
        const wVals = srcArrays.map(() => []); // giá trị mỗi array theo welded index
        const buckets = new Map();             // cellKey -> [welded idx]
        const cellOf = (x) => Math.floor(x / eps);
        const cKey = (a, b2, c) => `${a},${b2},${c}`;

        const weld = ({ p, vals }) => {
            const ci = cellOf(p[0]), cj = cellOf(p[1]), ck = cellOf(p[2]);
            // Dò cả 27 ô lân cận -> điểm cách <= eps được coi là trùng.
        ...
        // ---- Giao mặt phẳng với từng tam giác -> đoạn (đã dedupe) ----
        ...
        // Cơ sở 2D trên mặt phẳng để phân loại lỗ + ghép vòng theo góc.
        ...
        // Boundary edge: xuất vòng kín thành polyline (điểm cuối nối lại điểm đầu).
        if (this.edges || !this.fill) {
            out.lines = loops.length
                ? loops.map(loop => [...loop, loop[0]])
                : segments.map(s => [s[0], s[1]]); // fallback nếu vòng hở
        }

        if (!this.fill) return out;
        if (loops.length === 0) { out.polys = []; return out; }

        // ---- Phân loại outer/hole theo độ sâu bao chứa (parity) ----
        ...
        // ---- earcut từng outer + holes của nó ----
        const outPolys = [];
        for (const o of outers) {
        ...
        // Normals cho mặt cắt (đều bằng pháp tuyến mặt phẳng).
...
// Nối các đoạn (cặp welded index) thành vòng kín.
// Tại giao điểm bậc > 2, chọn cạnh RẼ theo góc (face-tracing) để tách đúng vòng.
function _assembleLoops(segments, to2D) {
    ...
    for (const [s0, s1] of segments) {
        ...
        while (guard++ < max) {
            loop.push(cur);
            if (cur === s0) { loop.pop(); break; } // đã đóng vòng

            const nbrs = (adj.get(cur) || []).filter(x => !used.has(ek(cur, x)));
            if (nbrs.length === 0) break; // vòng hở
            let next;
            if (nbrs.length === 1) {
                next = nbrs[0];
            } else {
                // Đến cur từ prev; chọn cạnh có góc rẽ CCW nhỏ nhất so với hướng quay lại prev.
