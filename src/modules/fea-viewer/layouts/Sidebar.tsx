...
export default function Sidebar({ sceneController, sceneVersion, theme }) {
    ...
    useEffect(() => {
        const syncFromScene = (event: Event) => {
            const detail = (event as CustomEvent<{ actors?: any[]; activeActor?: any }>).detail;
            const actors = (detail?.actors ?? []).filter((actor) => actor?.isActor);
            setSelectedIds(actors.map((actor) => actor.uuid));
            setActiveActor(detail?.activeActor ?? actors.at(-1) ?? null);
        };
        window.addEventListener("fea-scene-selection-changed", syncFromScene);
        return () => window.removeEventListener("fea-scene-selection-changed", syncFromScene);
    }, []);
